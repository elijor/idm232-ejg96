    ...
    <main>
        ...
        <div class="mainsect column flex">
            ...
            if (isset($_GET['id'])) {
                ...
                $results = mysqli_query($db_connection, $query);
                while ($oneRecipe = mysqli_fetch_array($results)) {
                    $fixedSteps = str_replace(':*', ': ', $oneRecipe['All Steps']);
                    $fixedSteps2 = '1*' . substr($fixedSteps, 2);
                    $fixedSteps3 = preg_replace('/\*(\d+)\b/', '*$1*', $fixedSteps2);
                    $fixedSteps4 = str_replace('**', '*',  $fixedSteps3);
                    $fixedSteps5 = str_replace('"&"', '',  $fixedSteps4);
                    
                    $originalSteps2 = explode('*', $fixedSteps5);
                    $stepImgsArray = explode("*", $oneRecipe['Step IMGs']);
                    consoleMsg($originalSteps2[2]);
